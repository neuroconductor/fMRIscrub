...
#' clev = clever(X)
clever = function(
  X, measure="leverage", projection="PCA", PESEL=TRUE, solve_PC_dirs=FALSE,
  ...
  # Run `clever_multi`.
  clev <- clever_multi(
    X=X, measures=measure, projections=projection, solve_PC_dirs=solve_PC_dirs,
    center_X=center_X, scale_X=scale_X, detrend_X=detrend_X, PCATF_kwargs=PCATF_kwargs,
    kurt_quantile=kurt_quantile, get_outliers=get_outliers, outlier_cutoffs=outlier_cutoff,
