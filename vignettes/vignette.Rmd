...
```{r}
clever.Dat1.var.lev = clever(Dat1, verbose=TRUE, measures="leverage")
clever.Dat2.var.lev = clever(Dat2, verbose=TRUE, measures="leverage")
```
...
PCA leverage indicates clear outliers in the first dataset around the 60th and 150th timepoints. While the second dataset is relatively artifact-free, PCA leverage does indicate a few potential outliers including around the 40th and 110th timepoints.

Next, we can run all methods included in `clever` and compare the results:

```{r}
clever.Dat1 = clever::multi(Dat1, verbose=TRUE, measures="all", projections="all")
clever.Dat2 = clever::multi(Dat2, verbose=TRUE, measures="all", projections="all")
```

Here are the outliers for the first dataset:

```{r fig.height=22, fig.width=8}
plot(clever.Dat1, "all", title="Dat1")
```

And for the second:

```{r fig.height=22, fig.width=8}
plot(clever.Dat2, "all", title="Dat2")
```

For the first dataset, most methods identify outliers around timepoints 60 and 150 consistently. A few other timepoints are flagged as well for scrubbing and DVARS, and robust distance flags many more timepoints. For the second dataset, weaker outliers are identified around timepoint 40 and 110 inconsistently, especially for the kurtosis method. Overall, these results are consistent with our prior knowledge of both datasets. 
...
par(mfrow=c(1,2))
levs = clever.Dat1$measures$leverage__PCA_kurt
...
**This section is from 2.0 and needs to be revised for 3.0**
