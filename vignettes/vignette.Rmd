...
We next run `clever` on both datasets. We'll use the default implementation of PCA leverage:
...
To validate our results, we can reconstruct the original fMRI images using the mask which was applied to vectorize the data. See `clever::unmask_vol` for a helper function to do this.
...
clever.Dat1.all <- clever:::clever_multi(Dat1, projection="all", verbose=TRUE, comps_mean_dt=1, comps_var_dt=1)
