...
```{r}
clever.Dat1.var.lev = clever(Dat1, verbose=TRUE, DVARS=FALSE, lev_images=FALSE, PCs_detrend=FALSE)
clever.Dat2.var.lev = clever(Dat2, verbose=TRUE, DVARS=FALSE, lev_images=FALSE, PCs_detrend=FALSE)
```
...
Here's the same plots but with detrending the PCs before computing the outlyingness measurement:
...
```{r fig.width=8, fig.height=5}
plot_grid(plot(clever.Dat1.var.lev, plot_title="Dat1 w/ PC detrending"), plot(clever.Dat2.var.lev, plot_title="Dat2 w/ PC detrending"), nrow=1)
```

We could also use all combinations of projection and outlyingness methods and compare them (replace PCs_detrend with TRUE to see other result):

```{r}
clever.Dat1 = clever(Dat1, projection_methods="all", outlyingness_methods="all", verbose=TRUE, PCs_detrend=FALSE)
clever.Dat2 = clever(Dat2, projection_methods="all", outlyingness_methods="all", verbose=TRUE, PCs_detrend=FALSE)
```
